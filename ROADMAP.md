...
- **v0.7.0** (2026-07-24) - preview resilience and hardening. `LivePreviewCallback` now isolates runtime preview failures by default and offers `on_error="raise"` for strict callers. All built-in weight files have immutable revisions and role-specific sha256 pins; cache keys include a converter-format version; CI uses the frozen lockfile. Packed-latent validation, conversion layout handling, error messages, and kernel metadata are hardened. The six-scenario showcase now isolates live workers and records durable memory or wall-budget aborts. Its schema records generation and decode dtypes separately. The release report completed all scenarios with no regression against v0.6.2.
