...
- **v0.8.2** (2026-09-17) — TAEF2 fidelity fix. `LivePreviewCallback` no longer applies the Flux2VAE batch-norm inverse before TAEF2 when `flux=model` is passed: TAEF2 was distilled on the normalized latent, and the extra step made previews dark and oversaturated. SSIM against the full VAE on the benchmark latent rises from 0.616 to 0.960 (LPIPS 0.216 to 0.057) under the webp-scored showcase protocol. `auto_bn` defaults to `False`; opting back in still works and logs a warning. The FLUX.2 rows and frames in COMPARISON and EXAMPLES are re-measured, and the FLUX.2 and Qwen-Image unpack tests now compare against mflux's own inverse element for element.
