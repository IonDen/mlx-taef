...
- **v0.6.1** (2026-06-28) — test-suite hardening, no runtime or API change (the wheel is identical to v0.6.0). The parity-oracle fixtures are now SHA-256 pinned, so an accidental change to the reference outputs fails CI rather than silently weakening parity; the default `pytest` run is fully offline; and a number of tests were tightened to assert observable output and pin exact values, with added offline coverage for the Qwen-Image decode and conversion paths.
