...
- mlx-taef at commit `b94b52e` (the pre-tag dev build that produced the committed report; v0.6.2 measures the decode step in isolation and at steady state)
- Quantization: int4 (mflux `quantize=4`), activation dtype bf16
- All conditions ran in isolated subprocesses with `mx.set_wired_limit` set per the cap column, and each decode was timed after one untimed warmup call so the figure reflects steady-state per-step decode rather than a cold first call. Hardware metadata is recorded inline in `_artifacts/showcase_report.json` for auditability.
...
|---|---|---|
| Decode latency (median of 3/5 warmed subprocess reps) | 0.281 s | 0.031 s |
| Decode latency range | 0.279 – 0.282 s | 0.030 – 0.032 s |
...
**TAEF2 is ~9.2× faster, with ~4.8× lower peak decode memory.** SSIM(TAEF2, Vanilla) = **0.616** (15/15 pairs).
...
| Decode latency range | 0.298 – 0.302 s | 0.029 – 0.031 s |
...
**TAEF1 is ~10.2× faster, with ~6.8× lower peak decode memory.** SSIM(TAEF1, Vanilla) = **0.939** (15/15 pairs).
...
|---|---|---|
| Decode latency (median of 3/5 warmed subprocess reps) | 0.235 s | 0.0295 s |
| Decode latency range | 0.233 – 0.238 s | 0.029 – 0.031 s |
...
**TAEF1 is ~8.0× faster on the Z-Image latent, with ~4.8× lower peak decode memory.** SSIM(TAEF1, Vanilla) = **0.940** (15/15 pairs) — consistent with the FLUX.1 result above, since it's the same decoder.
...
- Wall-clock: **11.38 s** total (model load + 4 generation steps + 4 TAEF2 previews + final VAE decode)
...
- Wall-clock: **44.16 s** total (thermally sensitive — see the note below)
...
The much higher wall-clock and peak memory next to `live_preview` come from Z-Image-Turbo's own generation cost, not the preview decoder — its 25.9 GB working set is the dominant load here. This scenario is also thermally sensitive: run back-to-back after the FLUX scenarios, the sustained load pushes the M1 Max into throttling (per-step time climbs across the run), so a cold-start Z-Image generation is faster than the 44 s above. Either way the cost is Z-Image-Turbo's transformer, not the ~30 ms TAEF1 preview decode measured above, which holds regardless of which model produced the latent.
...
- Wall-clock: **8.77 s** total (vs `live_preview`'s 11.38 s → **1.30× speedup**)
- Peak memory: **5.70 GB** (vs 10.66 GB → **47% less**)
...
- The 47% peak-memory drop is partly the skipped transformer call (whose activations never materialise) and partly the mflux compiled-path interaction noted in mlx-teacache's own release notes. Be careful attributing it all to one cause.
