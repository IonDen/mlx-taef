...
These frames decode the in-loop latent and come from the 2026-08-09 capture under mflux 0.18.1.
mflux 0.19 changed the Krea 2 sigma schedule, so re-running the capture on 0.19 or 0.20 gives
different intermediate frames until the next imagery refresh. On mflux 0.20 the callback decodes
Krea 2's prediction of the finished image by default, which is recognizable from step 1; the
capture script passes `preview_source="latents"` so a re-capture keeps showing the in-loop latent.
