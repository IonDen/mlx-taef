...
The gain: decoding the final Z-Image latent takes **62 ms** with TAEF1 versus **2.76 s** with the
full Z-Image VAE, about **44× faster**, and it peaks at **0.55 GB** instead of 1.96 GB. Structural
...
The gain: TAEF2 decodes a Klein latent in **258 ms** versus **2.14 s** for the full FLUX.2 VAE
(~8.3× faster), at **0.59 GB** versus 2.62 GB peak. SSIM here is **0.616**, lower than the FLUX.1
...
TAEF1 decodes the same latent in **183 ms** versus **2.01 s** for the full FLUX.1 VAE (~11× faster),
at **0.55 GB** versus 3.00 GB peak and **SSIM 0.94**. Reproduce:
...
at roughly half a gigabyte (0.55–0.59 GB), while the full VAEs run 2–3 GB. On a 32 GB Mac shared
