...
- **v0.8.2 — released on PyPI** (2026-09-17). TAEF2 previews now match the full FLUX.2 VAE much more closely: SSIM against the full VAE decode rises from 0.616 to 0.960 on the benchmark latent. Earlier releases applied the VAE's batch-norm inverse to the latent before TAEF2 whenever `flux=model` was passed, and TAEF2 was never trained on that input, so previews came out dark and oversaturated. The default now feeds TAEF2 the latent as mflux produces it; `auto_bn=True` or explicit `bn_mean` / `bn_var` keep the old behavior and log a warning. COMPARISON and EXAMPLES carry the re-measured FLUX.2 numbers and frames.
