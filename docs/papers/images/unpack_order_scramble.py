...
OUT = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).with_name("unpack-order-scramble.png")

H = W = 96  # spatial size of the 32-channel latent (image would be 8x = 768 px)

def picture(seed_shift=0, hue=0):
    ...
    for i in range(6):
        d.rectangle([4 + i * 6, 80, 8 + i * 6, 92], fill=(255, 255, 255) if i % 2 == 0 else (0, 0, 0))
    for x in range(72, 96, 4):
        for y in range(0, 24, 4):
            d.rectangle([x, y, x + 3, y + 3], fill=(20, 20, 20) if ((x + y) // 4) % 2 == 0 else (230, 230, 230))
...
ra = np.array(route_a); rb = np.array(route_b)
print("route A exact:", np.array_equal(ra, latent))
print("route B exact:", np.array_equal(rb, latent), "max abs err:", float(np.abs(rb - latent).max()))
print("route B shape:", rb.shape)

def to_rgb(arr):  # (1, 32, H, W) -> uint8 (H, W, 3) from channels 0-2
    a = arr[0, :3].transpose(1, 2, 0)
    return ((np.clip(a, -1, 1) + 1) * 127.5).astype(np.uint8)

scale = 3
panels = [("Input latent (channels 0-2 as RGB)", to_rgb(latent)),
          ("Unpacked with the FLUX.2 order", to_rgb(ra)),
          ("Unpacked with the Ideogram 4 order", to_rgb(rb))]
pw, ph = W * scale, H * scale
pad, top = 16, 30
canvas = Image.new("RGB", (len(panels) * pw + (len(panels) + 1) * pad, ph + top + pad), (24, 24, 24))
